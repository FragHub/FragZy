...
public partial class FragBot3
{
    ...
    public void OnForceReadyCommandCommand(CCSPlayerController? player, CommandInfo? command)
    {
        ...
        if (playerCount < minReady) 
        {
            ...
            ReplyToUserCommand(player, Localizer["fragbot3.rs.minreadyplayers", minReady]);
            return;
        }
        ...
        foreach (var key in playerData.Keys)
        {
            ...
            if (playerData[key].TeamNum == player.TeamNum) {
                ...
                ReplyToUserCommand(playerData[key], Localizer["fragbot3.rs.forcereadiedby", player.PlayerName]);
            }
        }
