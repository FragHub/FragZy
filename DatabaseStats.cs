    ...
    public class Database
    {
        ...
        public void InitializeDatabase(string directory)
        {
            ...
            try
            {
                ...
                // Create the `fragbot3_stats_matches`, `fragbot3_stats_players` and `fragbot3_stats_maps` tables if they doesn't exist
                ...
                Log("[InitializeDatabase] Table fragbot3_stats_matches created (or already exists)");
                Log("[InitializeDatabase] Table fragbot3_stats_players created (or already exists)");
                Log("[InitializeDatabase] Table fragbot3_stats_maps created (or already exists)");
            }
            catch (Exception ex)
            {
        ...
        public void ConnectDatabase(string directory)
        {
            try
            {
                ...
                if (databaseType == DatabaseType.SQLite)
                {
                    connection =
                        new SqliteConnection(
                            $"Data Source={Path.Join(directory, "fragbot3.db")}");
                }
                ...
                else
                {
                    ...
                    connection = new SqliteConnection($"Data Source={Path.Join(directory, "fragbot3.db")}");
        ...
        {
            connection.Execute($@"
            CREATE TABLE IF NOT EXISTS fragbot3_stats_matches (
            ...
            connection.Execute(@"
                CREATE TABLE IF NOT EXISTS fragbot3_stats_maps (
                    ...
                    FOREIGN KEY (matchid) REFERENCES fragbot3_stats_matches (matchid)
                )");

            connection.Execute(@"
                CREATE TABLE IF NOT EXISTS fragbot3_stats_players (
                    matchid INTEGER NOT NULL,
                    mapnumber INTEGER NOT NULL,
                    ...
                    FOREIGN KEY (matchid) REFERENCES fragbot3_stats_matches (matchid),
                    FOREIGN KEY (matchid, mapnumber) REFERENCES fragbot3_stats_maps (matchid, mapnumber)
                )");
        }
        ...
        public void CreateRequiredTablesSQL()
        {
            ...
            CREATE TABLE IF NOT EXISTS fragbot3_stats_maps (
                ...
                CONSTRAINT fragbot3_stats_maps_matchid FOREIGN KEY (matchid) REFERENCES fragbot3_stats_matches (matchid)
            )");
            ...
            CREATE TABLE IF NOT EXISTS fragbot3_stats_players (
                ...
                PRIMARY KEY (matchid, mapnumber, steamid64),
                FOREIGN KEY (matchid) REFERENCES fragbot3_stats_matches (matchid),
                FOREIGN KEY (mapnumber) REFERENCES fragbot3_stats_maps (mapnumber)
            )");
        }
            ...
            try
            {
                ...
                if (mapNumber == 0) {
                    if (isMatchSetup && liveMatchId != -1) {
                        connection.Execute(@"
                            INSERT INTO fragbot3_stats_matches (matchid, start_time, team1_name, team2_name, series_type, server_ip)
                        ...
                        connection.Execute(@"
                            INSERT INTO fragbot3_stats_matches (start_time, team1_name, team2_name, series_type, server_ip)
                ...
                if (isMatchSetup && liveMatchId != -1) {
                    connection.Execute(@"
                        INSERT INTO fragbot3_stats_maps (matchid, start_time, mapnumber, mapname)
                ...
                Log($"[InsertMatchData] Data inserted into fragbot3_stats_matches with match_id: {matchId}");
                return matchId;
            }
            catch (Exception ex)
            {
        ...
        public async Task UpdatePlayerStatsAsync(long matchId, int mapNumber, Dictionary<ulong, Dictionary<string, object>> playerStatsDictionary)
        {
            try
            {
                foreach (ulong steamid64 in playerStatsDictionary.Keys)
                {
                    ...
                    string sqlQuery = $@"
                    INSERT INTO fragbot3_stats_players (
                    ...
                    if (connection is SqliteConnection) {
                        sqlQuery = @"
                        INSERT OR REPLACE INTO fragbot3_stats_players (
                            matchid, mapnumber, steamid64, team, name, kills, deaths, damage, assists,
                            enemy5ks, enemy4ks, enemy3ks, enemy2ks, utility_count, utility_damage,
                            utility_successes, utility_enemies, flash_count, flash_successes,
                            health_points_removed_total, health_points_dealt_total, shots_fired_total,
            ...
            try {
                ...
                using (var csv = new CsvWriter(writer, new CsvConfiguration(CultureInfo.InvariantCulture)))
                {
                    IEnumerable<dynamic> playerStatsData = await connection.QueryAsync(
                        "SELECT * FROM fragbot3_stats_players WHERE matchid = @MatchId AND mapnumber = @MapNumber ORDER BY team, kills DESC", new { MatchId = matchId, MapNumber = mapNumber });
        ...
        private void SetDatabaseConfig(string directory)
        {
            ...
            string configFile = Path.Combine(Server.GameDirectory + "/csgo/cfg/FragBot3", fileName);
