...
namespace FragBot3
{
    public partial class FragBot3
    {
        ...
        public string lastFragBot3BackupFileName = "";
        ...
        {
            string backupFilePrefix = $"fragbot3_{liveMatchId}_{matchConfig.CurrentMapNumber}";
        ...
        public void OnStopCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (isStopCommandAvailable && isMatchLive)
            {
                if (IsHalfTimePhase())
                {
                    ...
                    ReplyToUserCommand(player, Localizer["fragbot3.backup.stopduringhalftime"]);
                    return;
                }
                if (IsPostGamePhase())
                {
                    // ReplyToUserCommand(player, "You cannot use this command after the game has ended.");
                    ReplyToUserCommand(player, Localizer["fragbot3.backup.stopmatchended"]);
                    return;
                }
                if (IsTacticalTimeoutActive())
                {
                    // ReplyToUserCommand(player, "You cannot use this command when tactical timeout is active.");
                    ReplyToUserCommand(player, Localizer["fragbot3.backup.stoptacticaltimeout"]);
                    return;
                }
                if (playerHasTakenDamage && stopCommandNoDamage.Value)
                {
                    ReplyToUserCommand(player, Localizer["fragbot3.restore.stopcommandrequiresnodamage"]);
                    return;
                }
                string stopTeamName = "";
                ...
                {
                    if (lastFragBot3BackupFileName != "")
                    {
                        RestoreRoundBackup(player, lastFragBot3BackupFileName);
                    }
                    else
                    {
                        // This should not happen, lastFragBot3BackupFileName should not be empty in a live game!
                        Log($"[OnStopCommand] lastFragBot3BackupFileName not found, unable to restore round!");
                    }

                }
                else
                {
                    PrintToAllChat(Localizer["fragbot3.restore.teamwantstorestore", stopTeamName, remainingStopTeam]);
                    // Server.PrintToChatAll($"{chatPrefix} {ChatColors.Blue}{stopTeamName}{ChatColors.Default} wants to restore the game to the beginning of the current round. {ChatColors.Blue}{remainingStopTeam}{ChatColors.Default}, please write !stop to confirm.");
                }
            }
        }
        ...
        public void OnRestoreCommand(CCSPlayerController? player, CommandInfo command)
        {
            ...
            {
                ReplyToUserCommand(player, Localizer["fragbot3.cc.usage", "!restore <round>"]);
            }
        }
        ...
        private void HandleRestoreCommand(CCSPlayerController? player, string commandArg)
        {
            ...
            if (!string.IsNullOrWhiteSpace(commandArg))
            {
                if (int.TryParse(commandArg, out int roundNumber) && roundNumber >= 0)
                {
                    ...
                    string requiredBackupFileName = $"fragbot3_{liveMatchId}_{matchConfig.CurrentMapNumber}_round{round}.json";
                ...
                else
                {
                    ...
                    ReplyToUserCommand(player, Localizer["fragbot3.backup.restoreinvalidvalue"]);
                }
            }
            else
            {
        ...
        private void RestoreRoundBackup(CCSPlayerController? player, string fileName)
        {
            ...
            if (IsHalfTimePhase())
            {
                ReplyToUserCommand(player, Localizer["fragbot3.backup.restoreduringhalftime"]);
                return;
            }
            if (IsPostGamePhase())
            {
                ReplyToUserCommand(player, Localizer["fragbot3.backup.restorematchended"]);
                return;
            }
            if (IsTacticalTimeoutActive())
            {
                ReplyToUserCommand(player, Localizer["fragbot3.backup.restoretacticaltimeout"]);
                return;
            }
            string backupFolder = Path.Combine(Server.GameDirectory, "csgo", "FragBot3DataBackup");
     
            string filePath = Path.Combine(backupFolder, fileName);
 
            if (!File.Exists(filePath))
            {
                ReplyToUserCommand(player, Localizer["fragbot3.backup.restoredoesntexist", fileName]);
            ...
            try
            {
                ...
                {
                    fragbot3Team1 = Newtonsoft.Json.JsonConvert.DeserializeObject<Team>(team1config)!;
                }
                if (backupData.TryGetValue("team2", out var team2config))
                {
                    fragbot3Team2 = Newtonsoft.Json.JsonConvert.DeserializeObject<Team>(team2config)!;
                }
                ...
                if (gameRules.WarmupPeriod)
                {
                    if (!isRoundRestorePending)
                    {
                        isRoundRestorePending = true;
                        pendingRestoreFileName = fileName;
                        PrintToAllChat(Localizer["fragbot3.restore.loadedsuccessfully", fileName]);
                        return;
                    }
                    else
                    {
                ...
                if (backupData.TryGetValue("valve_backup", out var valveBackup))
                {
                    ...
                    {
                        tempFileName = $"fragbot3_{liveMatchId}_{matchConfig.CurrentMapNumber}_round{roundNumber}.txt";
                    }
            ...
            PrintToAllChat(Localizer["fragbot3.restore.restoredsuccessfully", fileName]);
        ...
        public void CreateFragBot3RoundDataBackup()
        {
            Log($"[CreateFragBot3RoundDataBackup] isRoundRestoring: {isRoundRestoring} isMatchLive: {isMatchLive}");
            ...
            try
            {
                ...
                string round = roundNumber.ToString("D2");
                string fragBot3BackupFileName = $"fragbot3_{liveMatchId}_{matchConfig.CurrentMapNumber}_round{round}.json";
                string filePath = Path.Combine(Server.GameDirectory, "csgo", "FragBot3DataBackup", fragBot3BackupFileName);
                ...
                string lastBackupFilePath = $"fragbot3_{liveMatchId}_{matchConfig.CurrentMapNumber}_round{round}.txt"; ;
                ...
                Dictionary<string, string> roundData = new()
                    {
                        ...
                        { "team1_name", fragbot3Team1.teamName },
                        { "team1_flag", fragbot3Team1.teamFlag },
                        { "team1_tag", fragbot3Team1.teamTag },
                        { "team1_side", teamSides[fragbot3Team1] },
                        { "team2_name", fragbot3Team2.teamName },
                        { "team2_flag", fragbot3Team2.teamFlag },
                        { "team2_tag", fragbot3Team2.teamTag },
                        { "team2_side", teamSides[fragbot3Team2] },
                        { "team1_score", t1score.ToString() },
                        { "team2_score", t2score.ToString() },
                        { "team1_series_score", fragbot3Team1.seriesScore.ToString() },
                        { "team2_series_score", fragbot3Team2.seriesScore.ToString() },
            ...
            {
                Log($"[CreateFragBot3RoundDataBackup FATAL] Error creating the JSON file: {e.Message}");
            }
        }

        public List<string> GetBackups(string matchID)
        {
            string backupDir = Path.Combine(Server.GameDirectory, "csgo", "FragBot3DataBackup");
            ...
            var pattern = $"fragbot3_{matchID}_";
        ...
        {
            Team teamConfig = team == "team1" ? fragbot3Team1 : fragbot3Team2;
        ...
        [ConsoleCommand("fragbot3_loadbackup", "Restore the backup from the provided file")]
        ...
        [ConsoleCommand("fragbot3_loadbackup_url", "Loads a backup from the given URL")]
        public void LoadBackupFromURL(CCSPlayerController? player, CommandInfo command)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    string filePath = Path.Combine(Server.GameDirectory, "csgo", "FragBot3DataBackup", fileName);
        ...
        [ConsoleCommand("fragbot3_listbackups", "List all the backups for the provided matchid")]
