    ...
    public partial class FragBot3
    {
        ...
        public void VetoCountdown()
        {
            ...
            if (warningsPrinted >= vetoCountdownTime)
            {
                ...
                Server.PrintToChatAll($"{chatPrefix} Captain for {ChatColors.Blue}{fragbot3Team1.teamName}{ChatColors.Default}: {ChatColors.Blue}{playerData[team1Captain].PlayerName}{ChatColors.Default}");
                Server.PrintToChatAll($"{chatPrefix} Captain for {ChatColors.Blue}{fragbot3Team2.teamName}{ChatColors.Default}: {ChatColors.Blue}{playerData[team2Captain].PlayerName}{ChatColors.Default}");
        ...
        public void PromptForMapSelectionInChat(string option) {
            ...
            {
                case "team1_ban":
                    action = $"{ChatColors.Blue}{fragbot3Team1.teamName}{ChatColors.Default} must now {ChatColors.Red}BAN{ChatColors.Default} a map.";
                    client = vetoCaptains["team1"];
                    stepMessage = $"Use .ban <map> to ban a map";
                    break;
                case "team2_ban":
                    action = $"{ChatColors.Blue}{fragbot3Team2.teamName}{ChatColors.Default} must now {ChatColors.Red}BAN{ChatColors.Default} a map.";
                    client = vetoCaptains["team2"];
                    stepMessage = $"Use .ban <map> to ban a map";
                    break;                                                       
                case "team1_pick":
                    action = $"{ChatColors.Blue}{fragbot3Team1.teamName}{ChatColors.Default} must now {ChatColors.Blue}PICK{ChatColors.Default} a map to play as map {matchConfig.Maplist.Count + 1}.";
                    client = vetoCaptains["team1"];
                    stepMessage = $"Use .pick <map> to pick a map.";
                    break;
                case "team2_pick":
                    action = $"{ChatColors.Blue}{fragbot3Team2.teamName}{ChatColors.Default} must now {ChatColors.Blue}PICK{ChatColors.Default} a map to play as map {matchConfig.Maplist.Count + 1}.";
                    client = vetoCaptains["team2"];
                    stepMessage = $"Use .pick <map> to pick a map.";
                    break;
            }
        ...
        public bool PickMap(string mapName, int team) {
            ...
            Team fragbot3Team = fragbot3Team1;

            if (team != 0) {
                fragbot3Team = (team == 2) ? reverseTeamSides["TERRORIST"] : reverseTeamSides["CT"];
                Server.PrintToChatAll($"{chatPrefix} {ChatColors.Blue}{fragbot3Team.teamName}{ChatColors.Default} picked {ChatColors.Blue}{mapRemovedName}{ChatColors.Default} as map {matchConfig.Maplist.Count + 1}");
            }

            matchConfig.Maplist.Add(mapRemovedName);

            var mapPickedEvent = new FragBot3MapPickedEvent
            {
                MatchId = liveMatchId,
                MapName = mapRemovedName,
                MapNumber = matchConfig.Maplist.Count,
                Team = (fragbot3Team == fragbot3Team1) ? "team1" : "team2",
            };
        ...
        public bool BanMap(string mapName, int team) 
        {
            ...
            if (team != 0) {
                fragbot3Team = (team == 2) ? reverseTeamSides["TERRORIST"] : reverseTeamSides["CT"];
                Server.PrintToChatAll($"{chatPrefix} {ChatColors.Blue}{fragbot3Team.teamName}{ChatColors.Default} banned {ChatColors.LightRed}{mapRemovedName}{ChatColors.Default}");
            }

            var mapMapVetoedEvent = new FragBot3MapVetoedEvent
            {
                MatchId = liveMatchId,
                MapName = mapRemovedName,
                Team = (fragbot3Team == fragbot3Team1) ? "team1" : "team2",
        ...
        {
            Team fragbot3Team = team == "team1" ? fragbot3Team1 : fragbot3Team2;
            int teamSide = teamSides[fragbot3Team] == "CT" ? 3 : 2;
        ...
        public void PromptForSideSelectionInChat(CsTeam team) {
            ...
            Team fragbot3Team = (team == CsTeam.CounterTerrorist) ? reverseTeamSides["CT"] : reverseTeamSides["TERRORIST"];
            string teamString = (fragbot3Team == fragbot3Team1) ? "team1" : "team2";
            
            Server.PrintToChatAll($"{chatPrefix} {ChatColors.Blue}{fragbot3Team.teamName}{ChatColors.Default} must now pick a side to play on {ChatColors.Blue}{mapName}{ChatColors.Default}");
        ...
        public void HandleSideChoice(CsTeam side, int client) {
            ...
            string pickingTeam = (team == fragbot3Team1) ? "team1" : "team2";
        ...
        public void PickSide(CsTeam side, string team) {
            ...
            Server.PrintToChatAll($"{chatPrefix} {ChatColors.Blue}{fragbot3Team.teamName}{ChatColors.Default} elected to start as {ChatColors.Blue}{sideFormatted}{ChatColors.Default} on {ChatColors.Blue}{mapName}{ChatColors.Default}.");

            var sidePickedEvent = new FragBot3SidePickedEvent
            {
                MatchId = liveMatchId,
                MapName = mapName,
                MapNumber = matchConfig.Maplist.Count,
        ...
        {
            Team startingVetoTeam = fragbot3Team1;
            if (lastVetoTeam == CsTeam.CounterTerrorist)
            {
                if (reverseTeamSides["CT"] == fragbot3Team1) startingVetoTeam = fragbot3Team2;
                if (reverseTeamSides["CT"] == fragbot3Team2) startingVetoTeam = fragbot3Team1;
            }
            else if (lastVetoTeam == CsTeam.Terrorist)
            {
                if (reverseTeamSides["TERRORIST"] == fragbot3Team1) startingVetoTeam = fragbot3Team2;
                if (reverseTeamSides["TERRORIST"] == fragbot3Team2) startingVetoTeam = fragbot3Team1;
            }
            switch (matchConfig.NumMaps)
            {
                case 1:
                    ...
                    for (int i = 0; i < numberOfBans; i++)
                    {
                        ...
                        i % 2 == 0
                            ? (startingVetoTeam == fragbot3Team1 ? "team1_ban" : "team2_ban")
                            : (startingVetoTeam == fragbot3Team1 ? "team2_ban" : "team1_ban"));
                    }
                    break;
                ...
                case 2:
                    ...
                    {
                        matchConfig.MapBanOrder.Add(startingVetoTeam == fragbot3Team1 ? "team1_pick"
                                                                        : "team2_pick");
                        matchConfig.MapBanOrder.Add(startingVetoTeam == fragbot3Team1 ? "team2_pick"
                                                                        : "team1_pick");
                    }
                    else
                    {
                        matchConfig.MapBanOrder.Add(startingVetoTeam == fragbot3Team1 ? "team1_ban"
                                                                        : "team2_ban");
                        matchConfig.MapBanOrder.Add(startingVetoTeam == fragbot3Team1 ? "team2_ban"
                                                                        : "team1_ban");
                        matchConfig.MapBanOrder.Add(startingVetoTeam == fragbot3Team1 ? "team1_pick"
                                                                        : "team2_pick");
                        matchConfig.MapBanOrder.Add(startingVetoTeam == fragbot3Team1 ? "team2_pick"
                ...
                default:
                    ...
                    {  // 7 >= 3 + 2
                        ...
                        for (int i = 0; i < numberOfPicks; i++)
                        {
                            matchConfig.MapBanOrder.Add(
                                matchConfig.MapBanOrder.Count % 2 == 0
                                ? (startingVetoTeam == fragbot3Team1 ? "team1_pick" : "team2_pick")
                                : (startingVetoTeam == fragbot3Team1 ? "team2_pick" : "team1_pick"));
                        }
