...
namespace FragBot3
{
    ...
    public partial class FragBot3
    {
        ...
        public const string practiceCfgPath = "FragBot3/prac.cfg";
        public const string dryrunCfgPath = "FragBot3/dryrun.cfg";
        ...
        public void StartPracticeMode()
        {
            ...
            Server.PrintToChatAll($" {ChatColors.Blue}Spawns: {ChatColors.Default}.spawn, .ctspawn, .tspawn, .bestspawn, .worstspawn");
            Server.PrintToChatAll($" {ChatColors.Blue}Bots: {ChatColors.Default}.bot, .nobots, .crouchbot, .boost, .crouchboost");
            Server.PrintToChatAll($" {ChatColors.Blue}Nades: {ChatColors.Default}.loadnade, .savenade, .importnade, .listnades");
            Server.PrintToChatAll($" {ChatColors.Blue}Nade Throw: {ChatColors.Default}.rethrow, .throwindex <index>, .lastindex, .delay <number>");
            Server.PrintToChatAll($" {ChatColors.Blue}Utility & Toggles: {ChatColors.Default}.clear, .fastforward, .last, .back, .solid, .impacts, .traj");
            Server.PrintToChatAll($" {ChatColors.Blue}Sides & Others: {ChatColors.Default}.ct, .t, .spec, .fas, .god, .dryrun, .break, .exitprac");
        }
        ...
        private void HandleSpawnCommand(CCSPlayerController? player, string commandArg, byte teamNum, string command)
        {
            ...
            if (!string.IsNullOrWhiteSpace(commandArg))
            {
                if (int.TryParse(commandArg, out int spawnNumber) && spawnNumber >= 1)
                {
                    ...
                    ReplyToUserCommand(player, Localizer["fragbot3.pm.movedtospawn", $"{spawnNumber + 1}/{spawnsData[teamNum].Count}"]);
                }
                else
                {
                    ...
                    ReplyToUserCommand(player, Localizer["fragbot3.pm.negativenumber"]);
                    return;
                }
            }
            else
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.cc.usage", $"!{command} <number>"]);
            }
        }
        ...
        private void HandleSaveNadeCommand(CCSPlayerController? player, string saveNadeName)
        {
            ...
            if (!string.IsNullOrWhiteSpace(saveNadeName))
            {
                ...
                string savednadesfileName = "FragBot3/savednades.json";
                ...
                try
                {
                    ...
                    if (savedNadesDict.ContainsKey(playerSteamID) && savedNadesDict[playerSteamID].ContainsKey(lineupName))
                    {
                        ...
                        if (savedNadesDict[playerSteamID][lineupName]["Map"] == currentMapName)
                        {
                            ...
                            ReplyToUserCommand(player, Localizer["fragbot3.pm.lineupissaved"]);
                            return;
                        }
                    }
                    ...
                    PrintToPlayerChat(player, Localizer["fragbot3.pm.lineupsavedsucces", lineupName]);
                    PrintToAllChat(Localizer["fragbot3.pm.playersavedlineup", player.PlayerName, $"{lineupName} {playerPos} {playerAngle}"]);
                }
            ...
            else
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.cc.usage", $".savenade <name>"]);
            }
        }
        ...
        private void HandleDeleteNadeCommand(CCSPlayerController? player, string saveNadeName)
        {
                ...
                try
                {
                    ...
                    if (savedNadesDict.ContainsKey(playerSteamID) && savedNadesDict[playerSteamID].ContainsKey(saveNadeName))
                    {
                        ...
                        if (lineupInfo.ContainsKey("Map") && lineupInfo["Map"] == Server.MapName)
                        {
                            ...
                            ReplyToUserCommand(player, Localizer["fragbot3.pm.lineupdeletesuccess", saveNadeName]);
                        }
                        else
                        {
                            // ReplyToUserCommand(player, $"Lineup '{saveNadeName}' not found on the current map!");
                            ReplyToUserCommand(player, Localizer["fragbot3.pm.nadenotfoundonmap", saveNadeName]);
                        }
                    }
                    else
                    {
                        // ReplyToUserCommand(player, $"Lineup '{saveNadeName}' not found!");
                        ReplyToUserCommand(player, Localizer["fragbot3.pm.lineupnotfound", saveNadeName]);
                    }
                }
                catch (JsonException ex)
                {
            ...
            else
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.cc.usage", $".delnade <name>"]);
            }
        }
        ...
        private void HandleImportNadeCommand(CCSPlayerController? player, string saveNadeCode)
        {
                    ...
                    if (parts.Length == 7)
                    {
                        ...
                        if (savedNadesDict.ContainsKey(playerSteamID) && savedNadesDict[playerSteamID].ContainsKey(lineupName))
                        {
                            ...
                            if (existingLineup.ContainsKey("Map") && existingLineup["Map"] == currentMapName)
                            {
                                ...
                                ReplyToUserCommand(player, Localizer["fragbot3.pm.lineupalreadyexists", lineupName]);
                                return;
                            }
                        }
                        ...
                        ReplyToUserCommand(player, Localizer["fragbot3.pm.lineupimportedsuccess"]);
                    }
                    else
                    {
                        // ReplyToUserCommand(player, $"Invalid code format. Please provide a valid code with name, pos, and ang.");
                        ReplyToUserCommand(player, Localizer["fragbot3.pm.lineupinvalidcode"]);
                    }
                }
                catch (JsonException ex)
                {
            ...
            else
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.cc.usage", $".importnade <code>"]);
            }
        }
        ...
        private void ListLineups(CCSPlayerController player, string steamID, string mapName, Dictionary<string, Dictionary<string, Dictionary<string, string>>> savedNadesDict, string nadeFilter)
        {
            ...
            else
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.pm.nosavedlineups", steamID]);
        ...
        private void HandleLoadNadeCommand(CCSPlayerController? player, string loadNadeName)
        {
            ...
            if (!string.IsNullOrWhiteSpace(loadNadeName))
            {
                ...
                try
                {
                        ...
                        if (savedNadesDict.ContainsKey(currentSteamID))
                        {
                            ...
                            if (savedNadesDict[currentSteamID].ContainsKey(nearestName))
                            {
                                ...
                                if (lineupInfo.ContainsKey("Map") && lineupInfo["Map"] == Server.MapName)
                                {
                                    ...
                                    // Print messages
                                    // ReplyToUserCommand(player, $"Lineup {ChatColors.Blue}{nearestName}{ChatColors.Default} loaded successfully!");
                                    ReplyToUserCommand(player, Localizer["fragbot3.pm.lineuploadedsuccess", nearestName]);

                                    if (!string.IsNullOrWhiteSpace(lineupDesc))
                                    {
                                        player.PrintToCenter($"{lineupDesc}");
                                        // ReplyToUserCommand(player, $"Description: {ChatColors.Blue}{lineupDesc}{ChatColors.Default}");
                                        ReplyToUserCommand(player, Localizer["fragbot3.pm.lineupdesc", lineupDesc]);
                                    }
                                ...
                                else
                                {
                                    ...
                                    ReplyToUserCommand(player, Localizer["fragbot3.pm.nadenotfoundonmap", nearestName]);
                    ...
                    if (!lineupFound && !lineupOnWrongMap)
                    {
                        ...
                        ReplyToUserCommand(player, Localizer["fragbot3.pm.nadenotfound", loadNadeName]);
                    }
                }
                catch (JsonException ex)
                {
            ...
            else
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.pm.loadnadenotfound"]);
            }
        }
        ...
        public void OnGodCommand(CCSPlayerController? player, CommandInfo? command)
        {
			...
			if(currentHP > 100)
			{
				...
				// ReplyToUserCommand(player, $"God mode disabled!");
                		ReplyToUserCommand(player, "God is " + Localizer["fragbot3.cc.disabled"]);
				return;
			}
			else
			{
				...
				// ReplyToUserCommand(player, $"God mode enabled!");
                		ReplyToUserCommand(player, "God is " + Localizer["fragbot3.cc.enabled"]);
				return;
			}
        }
        ...
        public void OnPracCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (matchStarted)
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.pm.pracmatchstarted"]);
                return;
            }
        ...
        public void OnDryRunCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (matchStarted)
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.pm.dryrunmatchstarted"]);
                return;
            }
            if (!isPractice)
            {
                // ReplyToUserCommand(player, "Dryrun can only be started in practice mode!");
                ReplyToUserCommand(player, Localizer["fragbot3.pm.dryrunnopractice"]);
                return;
            }
        ...
        public void OnSpawnCommand(CCSPlayerController? player, CommandInfo command)
        {
            ...
            else
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.cc.usage", $"!spawn <round>"]);
            }
        }
        ...
        public void OnCtSpawnCommand(CCSPlayerController? player, CommandInfo command)
        {
            ...
            else
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.cc.usage", $"!ctspawn <round>"]);
            }
        }
        ...
        private void SpawnBot(CCSPlayerController botOwner, bool crouch)
        {
            try 
            {
                ...
                if (!unusedBotFound) {
                    ...
                    PrintToAllChat(Localizer["fragbot3.pm.botlimit"]);
                }
        ...
        public HookResult OnPlayerSpawn(EventPlayerSpawn @event, GameEventInfo info)
        {
            ...
            if (matchStarted && (fragbot3Team1.coach.Contains(player!) || fragbot3Team2.coach.Contains(player!)))
            {
        ...
        private void SideSwitchCommand(CCSPlayerController player, CsTeam team) {
          if (team > CsTeam.None) {
            if(player.TeamNum == (byte)CsTeam.Spectator) {
              ...
              ReplyToUserCommand(player, Localizer["fragbot3.pm.spectatorbroken"]);
              return;
            }
        ...
        public bool IsValidPositionForLastGrenade(CCSPlayerController player, int position)
        {
            ...
            if (!lastGrenadesData.ContainsKey(userId) || lastGrenadesData[userId].Count <= 0)
            {
                ...
                PrintToPlayerChat(player, Localizer["fragbot3.pm.nothrownnades"]);
                return false;
            }
            ...
            if (lastGrenadesData[userId].Count < position)
            {
                ...
                PrintToPlayerChat(player, Localizer["fragbot3.pm.grenadehistory", $"{lastGrenadesData[userId].Count}"]);
                return false;
            }
        ...
        public void RethrowSpecificNade(CCSPlayerController player, string nadeType)
        {
            ...
            if (!nadeSpecificLastGrenadeData.ContainsKey(userId) || !nadeSpecificLastGrenadeData[userId].ContainsKey(nadeType))
            {
                ...
                PrintToPlayerChat(player, Localizer["fragbot3.pm.nothrownnadestype", nadeType]);
                return;
            }
        ...
        public void HandleBackCommand(CCSPlayerController player, string number)
        {
            ...
            if (!string.IsNullOrWhiteSpace(number))
            {
                if (int.TryParse(number, out int positionNumber) && positionNumber >= 1)
                {
                    if (IsValidPositionForLastGrenade(player, positionNumber))
                    {
                        ...
                        PrintToPlayerChat(player, Localizer["fragbot3.pm.tptogrenade", $"{positionNumber + 1}/{lastGrenadesData[userId].Count}"]);
                    }
                }
                else
                {
                    // PrintToPlayerChat(player, $"Invalid value for !back command. Please specify a valid non-negative number. Usage: !back <number>");
                    PrintToPlayerChat(player, Localizer["fragbot3.pm.backinvalidvalue"]);
                    return;
                }
            }
            else
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.pm.backtonumber", thrownCount]);
            }
        }
        ...
        public void HandleThrowIndexCommand(CCSPlayerController player, string argString)
        {
            ...
            if (string.IsNullOrEmpty(argString))
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.pm.throwindextonumber", thrownCount]);
                return;
            }
            ...
            foreach (string arg in argsList)
            {
                if (int.TryParse(arg, out int positionNumber) && positionNumber >= 1)
                {
                    if (IsValidPositionForLastGrenade(player, positionNumber))
                    {
                        ...
                        PrintToPlayerChat(player, Localizer["fragbot3.pm.throwgrenadehistory", $"{positionNumber + 1}/{lastGrenadesData[userId].Count}"]);
                    }
                }
                else
                {
                    // PrintToPlayerChat(player, $"'{arg}' is not a valid non-negative number for !throwindex command.");
                    PrintToPlayerChat(player, Localizer["fragbot3.pm.backnegativenumber", arg]);
                }
            }
        }
        ...
        public void HandleDelayCommand(CCSPlayerController player, string delay)
        {
            ...
            if (string.IsNullOrWhiteSpace(delay))
            {
                ...
                ReplyToUserCommand(player, Localizer["fragbot3.cc.usage", $"!delay <delay_in_seconds>"]);
                return;
            }
            ...
            if (float.TryParse(delay, out float delayInSeconds) && delayInSeconds > 0)
            {
                if (IsValidPositionForLastGrenade(player, 0))
                {
                    ...
                    PrintToPlayerChat(player, Localizer["fragbot3.pm.delaygrenade", $"{delayInSeconds:0.00}", $"{lastGrenadesData[userId].Count}"]);
                }
            }
            else
            {
                // PrintToPlayerChat(player, $"Delay of {delayInSeconds:0.00}s set for grenade of index: {lastGrenadesData[userId].Count}.);
                PrintToPlayerChat(player, Localizer["fragbot3.pm.delayvalidnumber", $"{delayInSeconds:0.00}", $"{lastGrenadesData[userId].Count}"]);
                return;
            }
        }
        ...
        public void OnRethrowCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            {
                // PrintToPlayerChat(player, $"You have not thrown any nade yet!");
                PrintToPlayerChat(player, Localizer["fragbot3.pm.notthrownnade"]);
                return;
            }
        ...
        public void OnLastIndexCommand(CCSPlayerController? player, CommandInfo? command)
        {
            ...
            if (IsValidPositionForLastGrenade(player!, 1))
            {
                ...
                PrintToPlayerChat(player!, Localizer["fragbot3.pm.indexlastgrenade", $"{lastGrenadesData[player!.UserId!.Value].Count}"]);
            } 
        }
